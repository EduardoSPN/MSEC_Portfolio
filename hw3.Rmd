...
We have `r NC_counties` rows of data in our output for 2017 homicides by county in North Carolina.
...
```{r california unsolved homicides}
...
```{r California Gun Homicide Investigations, cache=TRUE}
california <- murder %>%
                filter(state == 'California')
...
```{r California boxplot, cache=TRUE}
gun_types <- c('Firearm, type not stated', 'Handgun - pistol, revolver, etc', 'Other gun', 'Rifle', 'Shotgun')
california <- murder %>% 
